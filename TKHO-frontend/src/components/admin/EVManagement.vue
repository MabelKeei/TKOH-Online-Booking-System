    ...
    <div class="page-content">
      ...
      <el-table :data="paginatedData" height="100%" border stripe table-layout="auto" style="width: 100%">
        ...
        <el-table-column prop="type" label="Type" min-width="120">
          <template #default="{ row }">
            <el-tag :type="row.type === 'EV' ? 'success' : 'info'">{{ row.type }}</el-tag>
          </template>
        </el-table-column>
      ...
      <div class="pagination-bar">
        <div class="pagination-info">
          Showing {{ startIndex + 1 }}-{{ endIndex }} of {{ parkingList.length }} records
        </div>
...
const totalPages = computed(() => Math.max(1, Math.ceil(parkingList.value.length / pageSize.value)))
const startIndex = computed(() => (currentPage.value - 1) * pageSize.value)
const endIndex = computed(() => Math.min(startIndex.value + pageSize.value, parkingList.value.length))
...
const handleExport = () => {
  ...
  XLSX.writeFile(wb, `Parking_Management_${new Date().toISOString().split('T')[0]}.xlsx`)
  ElMessage.success('Excel file exported successfully')
}

const handleAdd = () => {
  formMode.value = 'add'
  formData.value = { slotNumber: '', location: '', type: 'EV', status: 'active' }
  showForm.value = true
}
...
const handleSave = () => {
  if (formMode.value === 'add') {
    parkingList.value.push({ ...formData.value, id: Date.now() })
    ElMessage.success('Parking slot added successfully')
  } else {
    const index = parkingList.value.findIndex(item => item.id === formData.value.id)
    if (index !== -1) {
      parkingList.value[index] = { ...formData.value }
      ElMessage.success('Parking slot updated successfully')
    }
  }
